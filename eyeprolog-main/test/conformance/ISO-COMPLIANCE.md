...
precedence, end tokens at EOF, clause head/body variable identity, sequential
`op/3` and `set_prolog_flag/2` preparation effects, integer-to-float
transcendental evaluation, `char_code/2` type precedence, `read_term/3` EOF
handling, mixed arithmetic comparison, `integer_rounding_function=toward_zero`,
`set_prolog_flag/2` variable errors, and `bagof/3` answer order.
...
Draft/editorial or deliberately controversial STC proposals (for example the
status of `(:-)/1-2` as protected procedures or deleting standalone if-then)
remain outside the Part 1 + Corrigenda 1-3 strict baseline until standardized or
adopted as an explicit compatibility extension.
