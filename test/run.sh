...
run 'list select query' bash -c './bin/eyelog --query "select(b, [a, b, c], Rest)" test/eyelog-syntax.pl | grep -q "select(b, \[a, b, c\], \[a, c\])."'
...
run 'n queens enumerates all 724 ten-queen solutions' bash -c '[ "$(./bin/eyelog --query "queens(10, Qs)" examples/n-queens.pl | wc -l)" -eq 724 ]'
...
run 'ev range worlds classifies harsh trips' bash -c './bin/eyelog examples/ev-range-worlds.pl | grep -Fq "triple(ev_range_worlds, status, expected_world_pattern)." && ./bin/eyelog examples/ev-range-worlds.pl | grep -Fq "triple(winter_highway, riskyInWorld, w0)." && ./bin/eyelog examples/ev-range-worlds.pl | grep -Fq "triple(heavy_delivery, riskyInWorld, w3)."'
run 'dairy energy balance classifies cows' bash -c './bin/eyelog examples/dairy-energy-balance.pl | grep -Fq "triple(dairy_energy_balance, strongestDeficit, early_lactation)." && ./bin/eyelog examples/dairy-energy-balance.pl | grep -Fq "triple(late_lactation, status, positive_energy_balance)."'
run 'field nitrogen balance finds leaching risk' bash -c './bin/eyelog examples/field-nitrogen-balance.pl | grep -Fq "triple(field_nitrogen_balance, highestLeachingRisk, sandy_high)." && ./bin/eyelog examples/field-nitrogen-balance.pl | grep -Fq "triple(low_input, status, under_supplied)."'
run 'epidemic policy recommends combined controls' bash -c './bin/eyelog examples/epidemic-policy.pl | grep -Fq "triple(epidemic_policy, recommendedPolicy, vaccination_and_masks)." && ./bin/eyelog examples/epidemic-policy.pl | grep -Fq "triple(vaccination_campaign, status, insufficient_control)."'
run 'nixon diamond keeps conflicting defaults explicit' bash -c './bin/eyelog examples/nixon-diamond.pl | grep -Fq "triple(nixon, status, conflicted_default_case)." && ./bin/eyelog examples/nixon-diamond.pl | grep -Fq "triple(nixon, defaultSupports, pacifist)." && ./bin/eyelog examples/nixon-diamond.pl | grep -Fq "triple(nixon, defaultSupports, hawk)."'
run 'deontic logic reports compensated and uncompensated violations' bash -c './bin/eyelog examples/deontic-logic.pl | grep -Fq "triple(alice, compensation, compensation(share_record, notify_dpo))." && ./bin/eyelog examples/deontic-logic.pl | grep -Fq "triple(alice, status, requires_review)."'
run 'gdpr compliance distinguishes compliant and noncompliant cases' bash -c './bin/eyelog examples/gdpr-compliance.pl | grep -Fq "triple(case_alpha, status, gdpr_compliant)." && ./bin/eyelog examples/gdpr-compliance.pl | grep -Fq "triple(case_beta, reason, transfer_without_adequacy)."'
...
run 'sat dpll solves larger eight-variable instance' bash -c './bin/eyelog examples/sat-dpll.pl | grep -Fq "triple(sat_instance, modelCount, 18)." && ./bin/eyelog examples/sat-dpll.pl | grep -Fq "triple(sat_instance, variableCount, 8)." && ./bin/eyelog examples/sat-dpll.pl | grep -Fq "triple(sat_instance, clauseCount, 12)."'
run 'hamiltonian cycle finds 72 cycles from fixed start' bash -c './bin/eyelog examples/hamiltonian-cycle.pl | grep -Fq "triple(hamiltonian_cycle, cycleCountFromA, 72)." && ./bin/eyelog examples/hamiltonian-cycle.pl | grep -Fq "triple(hamiltonian_cycle, witness, [a, b, c, d, e, f, g, h, a])."'
run 'cryptarithmetic solves classic and larger puzzles' bash -c './bin/eyelog examples/cryptarithmetic-send-more-money.pl | grep -Fq "triple(send_more_money, equation, plus(9567, 1085, 10652))." && ./bin/eyelog examples/cryptarithmetic-send-more-money.pl | grep -Fq "triple(donald_gerald_robert, equation, plus(526485, 197485, 723970))."'
run 'exact cover sudoku solves 9x9 candidate-row puzzle' bash -c './bin/eyelog examples/exact-cover-sudoku.pl | grep -Fq "triple(exact_cover_sudoku, firstRow, [5, 3, 4, 6, 7, 8, 9, 1, 2])." && ./bin/eyelog examples/exact-cover-sudoku.pl | grep -Fq "[3, 4, 5, 2, 8, 6, 1, 7, 9]"'
run 'blocks world planner finds five-block five-move plan' bash -c './bin/eyelog examples/blocks-world-planning.pl | grep -Fq "triple(blocks_world, blockCount, 5)." && ./bin/eyelog examples/blocks-world-planning.pl | grep -Fq "triple(blocks_world, plan, [move(e, d, table), move(d, c, table), move(c, b, table), move(d, table, c), move(e, table, d)])."'
section Examples
