...
run_api(){
  ...
  run 'short help flag' bash -c '"$0" -h | grep -Fq "usage: eyelog" && "$0" -h | grep -Fq -- "--query GOAL" && "$0" -h | grep -Fq -- "--stats"' "$BIN"
  ...
  run 'sudoku builtin solves third puzzle' bash -c '"$0" --query "once(solution(third, S))" examples/sudoku.pl | grep -Fq "solution(third, [[4, 7, 8, 2, 6, 9, 1, 3, 5]"' "$BIN"
...
N = 1000
for i in range(N):
    print(f"p{i}(a) :- p{i + 1}(a).")
print(f"p{N}(a).")
print("materialize(p0, 1).")
PY
"$0" "$TEST_TMPDIR/chain.pl" > "$TEST_TMPDIR/chain.out" && grep -Fxq "p0(a)." "$TEST_TMPDIR/chain.out"' "$BIN"
...
PY' "$BIN"
  ...
  run 'countall counts generated answers' bash -c 'printf "score(a,3).\nscore(b,1).\nscore(c,2).\n" > "$TEST_TMPDIR/aggregate.pl" && "$0" --query "countall(score(X, S), C)" "$TEST_TMPDIR/aggregate.pl" | grep -Fq "countall(score(X, S), 3)."' "$BIN"
  run 'sumall sums numeric templates' bash -c 'printf "score(a,3).\nscore(b,1).\nscore(c,2).\n" > "$TEST_TMPDIR/aggregate.pl" && "$0" --query "sumall(S, score(X, S), Sum)" "$TEST_TMPDIR/aggregate.pl" | grep -Fq "sumall(S, score(X, S), 6)."' "$BIN"
  run 'aggregate_min selects lowest key and template' bash -c 'printf "score(a,3).\nscore(b,1).\nscore(c,2).\n" > "$TEST_TMPDIR/aggregate.pl" && "$0" --query "aggregate_min(S, X, score(X, S), BestS, BestX)" "$TEST_TMPDIR/aggregate.pl" | grep -Fq "aggregate_min(S, X, score(X, S), 1, b)."' "$BIN"
  run 'aggregate_max selects highest key and template' bash -c 'printf "score(a,3).\nscore(b,1).\nscore(c,2).\n" > "$TEST_TMPDIR/aggregate.pl" && "$0" --query "aggregate_max(S, X, score(X, S), BestS, BestX)" "$TEST_TMPDIR/aggregate.pl" | grep -Fq "aggregate_max(S, X, score(X, S), 3, a)."' "$BIN"
