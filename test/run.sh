...
run 'delfour policy is projected from formula data' bash -c './bin/eyelog --query "policy_triple(:policy, odrl:permission, X)" examples/delfour.pl | grep -q "policy_triple(:policy, odrl:permission, permission(odrl:use, :insight, \"shopping_assist\"))."'
...
run 'log not equal query' bash -c './bin/eyelog --query "neq(dp:P1, dp:P2)" examples/dining-philosophers.pl | grep -q "neq(dp:P1, dp:P2)."'
...
run 'odrl policy formula is materialized as formula data' bash -c './bin/eyelog examples/odrl-dpv-risk-ranked.pl | grep -q "triple(:PolicyGraph1, :contains, triple(:Policy1, odrl:permission, :PermDeleteAccount))."'
...
run 'annotation names Alice triple directly' bash -c './bin/eyelog examples/annotation.pl | grep -q "triple(:t, log:nameOf, triple(:a, :name, \"Alice\"))."'
...
run 'context association keeps source formula names' bash -c './bin/eyelog examples/context-association.pl | grep -q "triple(skolem:g0, log:nameOf, triple(:Bob, foaf:name, \"Bob\"))."'
