...
why(
  bestPortfolio(portfolio2026, result(137, 75, 28, [solar, hvac, training, quality, recycling, cloud, security])),
  proof(
    goal(bestPortfolio(portfolio2026, result(137, 75, 28, [solar, hvac, training, quality, recycling, cloud, security]))),
    by(rule("project-portfolio-optimization.pl", clause(19))),
...
why(
  lowRiskTarget(portfolio2026, result(126, 74, 23, [solar, battery, hvac, training, quality, cloud])),
  proof(
    goal(lowRiskTarget(portfolio2026, result(126, 74, 23, [solar, battery, hvac, training, quality, cloud]))),
    by(rule("project-portfolio-optimization.pl", clause(20))),
...
why(
  feasibleCount(portfolio2026, 1390),
  proof(
    goal(feasibleCount(portfolio2026, 1390)),
    by(rule("project-portfolio-optimization.pl", clause(21))),
    bindings([binding("Count", 1390)]),
    uses([
      proof(
        goal(countall(feasiblePortfolio(_Selected, _Value, _Cost, _Risk), 1390)),
...
why(
  projectCount(portfolio2026, 12),
  proof(
    goal(projectCount(portfolio2026, 12)),
    by(rule("project-portfolio-optimization.pl", clause(22))),
    bindings([binding("Count", 12)]),
    uses([
      proof(
        goal(countall(project(_Name, _Value, _Cost, _Risk), 12)),
        by(builtin(countall, 2))
      )
    ])
  )
...
why(
  totalAvailableValue(portfolio2026, 268),
  proof(
    goal(totalAvailableValue(portfolio2026, 268)),
    by(rule("project-portfolio-optimization.pl", clause(23))),
    bindings([binding("Total", 268)]),
    uses([
      proof(
        goal(sumall(Value, project(_Name, Value, _Cost, _Risk), 268)),
        by(builtin(sumall, 3))
      )
    ])
  )
...
why(
  note(portfolio2026, "aggregate builtins combine with pruning and memoization to avoid sorted candidate bags"),
  proof(
    goal(note(portfolio2026, "aggregate builtins combine with pruning and memoization to avoid sorted candidate bags")),
    by(rule("project-portfolio-optimization.pl", clause(24))),
    uses([
      proof(
        goal(eq(ok, ok)),
        by(builtin(eq, 2))
