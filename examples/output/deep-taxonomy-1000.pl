...
why(
  ...
  proof(
    goal(is(test, true)),
    by(rule("deep-taxonomy-1000.pl", clause(8))),
    uses([
      proof(
        goal(once(a(ind, a2))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(a(ind, a2)),
            by(rule("deep-taxonomy-1000.pl", clause(9))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n1000)),
                by(rule("deep-taxonomy-1000.pl", clause(10))),
                bindings([binding("X", ind)]),
                uses([
                  proof(
                    goal(a(ind, n0)),
                    by(fact("deep-taxonomy-1000.pl", clause(7)))
                  )
                ])
              )
            ])
...
why(
  ...
  proof(
    ...
    by(rule("deep-taxonomy-1000.pl", clause(3013))),
    uses([
      proof(
        goal(once(a(ind, n0))),
        by(builtin(once, 1)),
...
why(
  ...
  proof(
    ...
    by(rule("deep-taxonomy-1000.pl", clause(3014))),
    uses([
      proof(
        goal(once(a(ind, n1))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(a(ind, n1)),
            by(rule("deep-taxonomy-1000.pl", clause(13))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n0)),
      ...
      proof(
        ...
        uses([
          proof(
            goal(a(ind, i1)),
            by(rule("deep-taxonomy-1000.pl", clause(14))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n0)),
      ...
      proof(
        ...
        uses([
          proof(
            goal(a(ind, j1)),
            by(rule("deep-taxonomy-1000.pl", clause(15))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n0)),
...
why(
  arc(check3, "C3 OK - the chain reaches the midpoint n500 and still carries both side-label branches."),
  proof(
    goal(arc(check3, "C3 OK - the chain reaches the midpoint n500 and still carries both side-label branches.")),
    by(rule("deep-taxonomy-1000.pl", clause(3015))),
    uses([
      proof(
        goal(once(a(ind, n500))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(a(ind, n500)),
            by(rule("deep-taxonomy-1000.pl", clause(1510))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n499)),
                by(rule("deep-taxonomy-1000.pl", clause(11))),
                bindings([binding("X", ind)]),
                uses([
                  proof(
                    goal(a(ind, n0)),
      ...
      proof(
        goal(once(a(ind, i500))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(a(ind, i500)),
            by(rule("deep-taxonomy-1000.pl", clause(1511))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n499)),
      ...
      proof(
        goal(once(a(ind, j500))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(a(ind, j500)),
            by(rule("deep-taxonomy-1000.pl", clause(1512))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n499)),
...
why(
  arc(check4, "C4 OK - the final taxonomy step from n999 to n1000 was completed."),
  proof(
    goal(arc(check4, "C4 OK - the final taxonomy step from n999 to n1000 was completed.")),
    by(rule("deep-taxonomy-1000.pl", clause(3016))),
    uses([
      proof(
        goal(once(a(ind, n999))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(a(ind, n999)),
            by(rule("deep-taxonomy-1000.pl", clause(12))),
            bindings([binding("X", ind)]),
            uses([
              proof(
                goal(a(ind, n0)),
...
why(
  arc(check5, "C5 OK - once n1000 is reached, the terminal class a2 is derived."),
  proof(
    goal(arc(check5, "C5 OK - once n1000 is reached, the terminal class a2 is derived.")),
    by(rule("deep-taxonomy-1000.pl", clause(3017))),
    uses([
      proof(
        goal(once(a(ind, n1000))),
        by(builtin(once, 1)),
...
why(
  ...
  proof(
    ...
    by(rule("deep-taxonomy-1000.pl", clause(3018))),
    uses([
      proof(
        goal(once(a(ind, a2))),
        by(builtin(once, 1)),
...
why(
  answer(report, "The test succeeds: starting from one individual classified as n0, the rules eventually classify it as n1000 and then as a2."),
  proof(
    goal(answer(report, "The test succeeds: starting from one individual classified as n0, the rules eventually classify it as n1000 and then as a2.")),
    by(rule("deep-taxonomy-1000.pl", clause(3019))),
    uses([
      proof(
        goal(once(is(test, true))),
        by(builtin(once, 1)),
...
why(
  reason(report, "The adjacent rules mirror the Eyeling N3 deep-taxonomy-1000 chain. Redundant checkpoint rules are included only as proof accelerators: each shortcut is entailed by the adjacent chain, and once/1 prevents backtracking into the long adjacent-only proof after a check has succeeded."),
  proof(
    goal(reason(report, "The adjacent rules mirror the Eyeling N3 deep-taxonomy-1000 chain. Redundant checkpoint rules are included only as proof accelerators: each shortcut is entailed by the adjacent chain, and once/1 prevents backtracking into the long adjacent-only proof after a check has succeeded.")),
    by(rule("deep-taxonomy-1000.pl", clause(3020))),
    uses([
      proof(
        goal(once(a(ind, a2))),
        by(builtin(once, 1)),
...
why(
  ...
  proof(
    ...
    by(rule("deep-taxonomy-1000.pl", clause(3021))),
...
why(
  ...
  proof(
    ...
    by(rule("deep-taxonomy-1000.pl", clause(3022))),
    ...
    uses([
      ...
      proof(
        goal(once(arc(check3, "C3 OK - the chain reaches the midpoint n500 and still carries both side-label branches."))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(arc(check3, "C3 OK - the chain reaches the midpoint n500 and still carries both side-label branches.")),
      ...
      proof(
        goal(once(arc(check4, "C4 OK - the final taxonomy step from n999 to n1000 was completed."))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(arc(check4, "C4 OK - the final taxonomy step from n999 to n1000 was completed.")),
      ...
      proof(
        goal(once(arc(check5, "C5 OK - once n1000 is reached, the terminal class a2 is derived."))),
        by(builtin(once, 1)),
        uses([
          proof(
            goal(arc(check5, "C5 OK - once n1000 is reached, the terminal class a2 is derived.")),
